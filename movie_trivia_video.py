"""Movie Trivia Shorts video — Pollinations.ai + TTS + baked-in Pillow text."""
...
from src.movie_trivia import generate_movie_trivia_script
...
def make_title_card(img, text):
    ...
    draw_text(img, "MOVIE TRIVIA", 28, H-120, center=True, color=(255,200,0))
    return img


def make_trivia_card(img, title, story):
    img = img.copy()
...
def make_end_card(img):
    ...
    draw_text(img, "FOR MORE MOVIE SECRETS", 36, H//2 + 20, center=True)
    return img
...
def main():
    print("="*50)
    print("  MOVIE TRIVIA GENERATOR")
    print("="*50)

    data = generate_movie_trivia_script()
    TITLE = data["title"]
    TITLES = data["trivia_titles"]
    STORIES = data["stories"]
    PROMPTS = data["image_prompts"]

    temp_dir = config.TEMP_DIR / "movie_trivia"
    ...
    for i, (title, prompt) in enumerate(zip(TITLES, PROMPTS)):
        cached = temp_dir / f"trivia_{i}.png"
    ...
    out = config.OUTPUT_DIR / f"movie_trivia_{safe_title}.mp4"
