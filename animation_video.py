...
from src.photo_video import generate_hf_text_to_video, generate_ai_image_video, generate_stock_photo_video, generate_photorealistic_frames, generate_coverr_video, generate_lorem_video, generate_pexels_video, generate_hf_space_video, generate_pollinations_video, generate_cogvideo, generate_openrouter_video, generate_freeai_video
...
def main():
    ...
    # Try 10: Pexels stock video (needs free API key from pexels.com)
    if bg is None and os.getenv("PEXELS_API_KEY"):
        print("  Try 10: Pexels stock video...")
        result, err = _run_with_timeout(generate_pexels_video, args=(user_prompt, video_path), timeout=30)
        if result:
            try:
                vid = VideoFileClip(str(video_path))
                if vid.duration < total_dur:
                    ...
                    vid = vid.with_duration(total_dur)
                bg = vid
                print("  >> Using Pexels stock video")
            except Exception:
                pass

    # Try 11: AI image Ken Burns (Pollinations images + zoom)
    if bg is None:
        print("  Try 11: AI image Ken Burns...")
        num_frames_needed = int(total_dur * FPS)
        result, err = _run_with_timeout(
    ...
    # Try 12: Stock photo Ken Burns (always works)
    if bg is None:
        print("  Try 12: Stock photo Ken Burns...")
        num_frames_needed = int(total_dur * FPS)
        result, err = _run_with_timeout(
