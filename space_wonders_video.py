"""Space Wonders Shorts video — Pollinations.ai + TTS + baked-in Pillow text."""
...
def make_title_card(img, text):
    ...
    draw_text(img, "SPACE WONDERS", 28, H-120, center=True, color=(100,150,255))
    return img
...
def make_fact_card(img, title, story):
    ...
    draw_text(img, title.upper(), 36, H-630, color=(100,150,255))
...
def make_end_card(img):
    ...
    draw_text(img, "SUBSCRIBE", 60, H//2 - 60, center=True, color=(100,150,255))
    draw_text(img, "FOR MORE SPACE FACTS", 36, H//2 + 20, center=True)
    return img
...
def main():
    print("="*50)
    print("  SPACE WONDERS GENERATOR")
    print("="*50)

    data = generate_space_wonders_script()
    TITLE = data["title"]
    TITLES = data["space_facts"]
    STORIES = data["stories"]
    PROMPTS = data["image_prompts"]

    temp_dir = config.TEMP_DIR / "space_wonders"
    ...
    for i, (title, prompt) in enumerate(zip(TITLES, PROMPTS)):
        cached = temp_dir / f"space_{i}.png"
    ...
    out = config.OUTPUT_DIR / f"space_{safe_title}.mp4"
