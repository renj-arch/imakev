"""Psychology Hacks Shorts video — Pollinations.ai + TTS + baked-in Pillow text."""
...
import config
from src.psychology import generate_psychology_script
...
def make_title_card(img, text):
    ...
    draw_text(img, "YOUR BRAIN SECRETS", 28, H-120, center=True, color=(255,200,0))
    return img


def make_hack_card(img, hack, explanation):
    img = img.copy()
    overlay = Image.new("RGBA", (W, int(H*0.45)), (0,0,0,180))
    img.paste(overlay, (0, H-int(H*0.45)), overlay)
    draw_text(img, hack.upper(), 42, H-520, color=(255,200,0))
...
def make_end_card(img):
    ...
    draw_text(img, "FOR MORE PSYCHOLOGY HACKS", 36, H//2 + 20, center=True)
    return img
...
def main():
    print("="*50)
    print("  PSYCHOLOGY HACKS GENERATOR")
    print("="*50)

    data = generate_psychology_script()
    TITLE = data["title"]
    HACKS = data["hacks"]
    EXPLANATIONS = data["explanations"]
    PROMPTS = data["image_prompts"]

    temp_dir = config.TEMP_DIR / "psychology"
    ...
    images = {}
    for i, (h, prompt) in enumerate(zip(HACKS, PROMPTS)):
        cached = temp_dir / f"psych_{i}.png"
    ...
    out = config.OUTPUT_DIR / f"psychology_{safe_title}.mp4"
