...
def _fallback() -> dict:
    ...
    items_data = [{"title": t, "description": d} for t, d, _ in items]
    return {
        ...
        "script": retention_tts(items_data, "satisfying", hook),
        "tts_script": retention_tts(items_data, "satisfying", hook),
    }
...
def _try_llm() -> dict | None:
    try:
        ...
        if topics and len(topics) >= 3:
            ...
            items_data = [{"title": t, "description": d} for t, d in zip(topics, descriptions)]
            return {
