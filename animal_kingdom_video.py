"""Animal Kingdom Shorts video — Pollinations.ai + TTS + baked-in Pillow text."""
...
def make_title_card(img, text):
    ...
    draw_text(img, "ANIMAL KINGDOM", 28, H-120, center=True, color=(0,200,100))
    return img
...
def make_fact_card(img, title, story):
    ...
    draw_text(img, title.upper(), 36, H-630, color=(0,200,100))
...
def make_end_card(img):
    ...
    draw_text(img, "SUBSCRIBE", 60, H//2 - 60, center=True, color=(0,200,100))
    draw_text(img, "FOR MORE AMAZING ANIMALS", 36, H//2 + 20, center=True)
    return img
...
def main():
    print("="*50)
    print("  ANIMAL KINGDOM GENERATOR")
    print("="*50)

    data = generate_animal_kingdom_script()
    TITLE = data["title"]
    TITLES = data["animal_facts"]
    STORIES = data["stories"]
    PROMPTS = data["image_prompts"]

    temp_dir = config.TEMP_DIR / "animal_kingdom"
    ...
    for i, (title, prompt) in enumerate(zip(TITLES, PROMPTS)):
        cached = temp_dir / f"animal_{i}.png"
