"""Life Hacks Shorts video — Pollinations.ai + TTS + baked-in Pillow text."""
...
def make_title_card(img, text):
    ...
    draw_text(img, "LIFE HACKS THAT ACTUALLY WORK", 28, H-120, center=True, color=(255,200,0))
    return img
...
def make_hack_card(img, hack, explanation):
    ...
    draw_text(img, hack.upper(), 38, H-460, color=(255,200,0))
    draw_text(img, explanation, 28, H-380)
    return img
...
def make_end_card(img):
    ...
    draw_text(img, "FOR MORE LIFE HACKS", 36, H//2 + 20, center=True)
    return img
...
def main():
    print("="*50)
    print("  LIFE HACKS GENERATOR")
    print("="*50)

    data = generate_life_hacks_script()
    TITLE = data["title"]
    HACKS = data["hacks"]
    EXPLANATIONS = data["explanations"]
    PROMPTS = data["image_prompts"]

    temp_dir = config.TEMP_DIR / "life_hacks"
