"""Unsolved Mysteries Shorts video — Pollinations.ai + TTS + baked-in Pillow text."""
...
from src.unsolved_mysteries import generate_unsolved_mysteries_script
...
def make_title_card(img, text):
    ...
    draw_text(img, "UNSOLVED MYSTERIES", 28, H-120, center=True, color=(180,30,30))
    return img
...
def make_story_card(img, title, story):
    ...
    draw_text(img, title.upper(), 36, H-630, color=(180,30,30))
...
def make_end_card(img):
    ...
    draw_text(img, "SUBSCRIBE", 60, H//2 - 60, center=True, color=(180,30,30))
    draw_text(img, "FOR MORE UNSOLVED CASES", 36, H//2 + 20, center=True)
    return img
...
def main():
    print("="*50)
    print("  UNSOLVED MYSTERIES GENERATOR")
    print("="*50)

    data = generate_unsolved_mysteries_script()
    TITLE = data["title"]
    TITLES = data["mysteries"]
    STORIES = data["stories"]
    PROMPTS = data["image_prompts"]

    temp_dir = config.TEMP_DIR / "unsolved_mysteries"
    ...
    out = config.OUTPUT_DIR / f"unsolved_{safe_title}.mp4"
