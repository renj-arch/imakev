"""Box Office Shorts video — Pollinations.ai + TTS + baked-in Pillow text."""
...
from src.box_office import generate_box_office_script
...
def make_title_card(img, text):
    ...
    draw_text(img, "BOX OFFICE FACTS", 28, H-120, center=True, color=(255,200,50))
    return img


def make_fact_card(img, title, story):
    img = img.copy()
    overlay = Image.new("RGBA", (W, int(H*0.55)), (0,0,0,200))
    img.paste(overlay, (0, H-int(H*0.55)), overlay)
    draw_text(img, title.upper(), 36, H-630, color=(255,200,50))
...
def make_end_card(img):
    ...
    draw_text(img, "SUBSCRIBE", 60, H//2 - 60, center=True, color=(255,200,50))
    draw_text(img, "FOR MORE MOVIE FACTS", 36, H//2 + 20, center=True)
    return img
...
def main():
    print("="*50)
    print("  BOX OFFICE GENERATOR")
    print("="*50)

    data = generate_box_office_script()
    TITLE = data["title"]
    TITLES = data["box_office_titles"]
    STORIES = data["stories"]
    PROMPTS = data["image_prompts"]

    temp_dir = config.TEMP_DIR / "box_office"
    ...
    out = config.OUTPUT_DIR / f"boxoffice_{safe_title}.mp4"
